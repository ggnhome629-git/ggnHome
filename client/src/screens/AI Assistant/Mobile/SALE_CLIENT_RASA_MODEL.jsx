...
const VoiceAssistantSale = () => {
  ...
  // Hybrid auth: get access token for Authorization header fallback
  ...
  const quickSuggestions = [
    ...
    ["₹50L-70L", "₹70L-1Cr", "₹1Cr-1.5Cr", "₹1.5Cr+"],
    ["Apartment", "Independent House", "Villa", "Plot"],
    ["2 BHK", "3 BHK", "4 BHK", "Penthouse"],
    ["Personal Use", "Investment", "Both"],
    ["Swimming Pool", "Gym", "Parking", "Security", "Garden"],
    ["Immediately", "Within 3 months", "Within 6 months", "Flexible"]
  ];
  ...
  const processUserResponse = async (userResponse) => {
    ...
    // If the recognized transcript mostly repeats the bot's question we usually want to ignore it
    // but allow short/numeric answers (e.g., "15k-20k", "2 bhk", "studio") even if they contain
    // some overlapping words. Also allow answers that clearly contain numbers/currency.
    const hasNumericAnswer = /\d+|₹|rs\b|k\b|bhk\b|studio\b|one\b|two\b|three\b/.test(normalizedUser);
    ...
    if (
      ...
      console.log("⚠️ Ignoring repeated input:", userResponse, { ratio, hasNumericAnswer });
      setMessages((prev) => [
        ...prev,
        {
          type: "bot",
    ...
    setTimeout(() => {
      ...
      if (nextIdx === questions.length - 1) {
        ...
        // Update suggestions for next question
  ...
  const handleSavePreferences = async () => {
    const orderedPrefs = {
      ...
      purchasePurpose: (collectedPrefsRef.current[4] || "")
        .toLowerCase()
        .includes("invest")
        ? "investment"
        : "personal",
    ...
    const searchQuery = encodeURIComponent(
      `${orderedPrefs.size || ""} in ${orderedPrefs.sector}`.trim()
    );
  ...
  return (
    <>
      <Helmet>
  <title>ARIA — AI Property Buying Assistant | GgnHome</title>

  <meta
    name="description"
    content="ARIA is GgnHome's AI assistant for property buyers in Gurgaon. Use voice to find flats, builder floors, villas, and luxury homes for sale. Get personalized sector recommendations, budget-based results, and AI-driven property insights."
  />

  <meta
    name="keywords"
    content="Gurgaon property for sale, buy house Gurgaon, AI real estate assistant, ARIA sale assistant, Gurgaon flats for sale, builder floors Gurgaon, sector 46 property sale, sector 57 Gurgaon sale, 2BHK for sale Gurgaon, 3BHK for sale Gurgaon"
  />

  {/* Open Graph (Preview on WhatsApp, LinkedIn, Facebook) */}
  <meta property="og:title" content="ARIA — AI Property Buying Assistant | GgnHome" />
  <meta
    property="og:description"
    content="Use ARIA to search Gurgaon properties for sale using voice. Personalized sector-based and budget-based recommendations powered by AI."
  />
  <meta property="og:image" content="https://ggnhome.in/logo192.png" />
  <meta property="og:url" content="https://ggnhome.in/AIassistant-Sale" />
  <meta property="og:type" content="website" />

  {/* Twitter */}
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:title" content="ARIA — AI Property Buying Assistant | GgnHome" />
  <meta
    name="twitter:description"
    content="Discover Gurgaon properties for sale with voice commands using ARIA."
  />
  <meta name="twitter:image" content="https://ggnhome.in/logo192.png" />

  {/* SEO Essentials */}
  <meta name="robots" content="index, follow" />
  <link rel="canonical" href="https://ggnhome.in/AIassistant-Sale" />
</Helmet>
      {/* Top Navigation Bar */}
      <div
        style={{
