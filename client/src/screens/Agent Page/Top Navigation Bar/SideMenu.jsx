...
export default function SideMenuBar({ currentUser, onLoginClick }) {
  ...
  const menuSections = [{title: null, items: [{icon: User, label: currentUser ? currentUser.email : 'LOGIN / REGISTER', color: '#0066FF', special: true, onClick: !currentUser ? onLoginClick : undefined}]}, {title: 'Quick Actions', items: [{icon: Home, label: 'Post Property', color: '#00A79D', badge: 'FREE', onClick: () => { if(currentUser){ window.location.href = '/agent/add-property'; } else { window.location.href = '/login'; } }}, {icon: Sparkles, label: 'AI Search', color: '#22D3EE', badge: 'NEW', link: '/AIassistant'}, {icon: Scale, label: 'Legal Laws', color: '#4A6A8A', onClick: () => {
    ...
    },
    {
      icon: Building,
      label: 'For Owners',
      onClick: () => { window.location.href = '/agent/add-property'; }
    },
    {
      icon: Users,
  ...
  <button
    ...
    onClick={() => {
      if (currentUser) {
        ...
        window.location.href = '/agent/add-property';
      } else {
