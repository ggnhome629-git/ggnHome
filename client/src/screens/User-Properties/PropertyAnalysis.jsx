...
export default function PropertyAnalytics() {
  ...
  // Hybrid authentication: get user token from localStorage (global step)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // PATCH 1: metrics fetch with hybrid auth
        ...
        // PATCH 2: conversion fetch with hybrid auth
  ...
  const styles = {
    container: { minHeight: '100vh', background: 'linear-gradient(135deg, #F4F7F9 0%, #FFFFFF 100%)', padding: '24px 12px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' },
    ...
    statsGrid: { maxWidth: '1400px', margin: '0 auto 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' },
  ...
  const handleLogout = async () => {
    // PATCH 4: logout with hybrid auth
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        // PATCH 3: fetchUser with hybrid auth
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={{ ...styles.header, marginTop: isMobile ? '80px' : '0' }}>
        <h1
          style={{
            ...styles.title,
            fontSize: isMobile ? '26px' : styles.title.fontSize
          }}
        >
      ...
      <div
        style={{
          ...styles.chartsGrid,
          gridTemplateColumns: isMobile ? '1fr' : styles.chartsGrid.gridTemplateColumns
        }}
      >
        <div
          style={{
            ...styles.chartCard,
            padding: isMobile ? '20px' : styles.chartCard.padding
          }}
        >
