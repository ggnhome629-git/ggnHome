...
const countdownRef = useRef(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Ensure the owner's contact number is sent with the form.
      // Prefer explicit formData.ownerMobile if provided; otherwise fall back to the logged-in user's phone fields.
      const ownerPhone = (formData.ownerMobile && String(formData.ownerMobile).trim())
        || (user && (user.mobile || user.phone || user.contact)) || "";
      if (ownerPhone) {
        form.append('ownerMobile', String(ownerPhone));
      }
      ...
      const url =
        formData.purpose === "Sale"
          ? `${process.env.REACT_APP_Base_API}/api/admin/addsaleproperties`
          : `${process.env.REACT_APP_Base_API}/api/admin/addrentproperties`;
...
/>
              ...
              <div style={fieldStyle}>
                <label style={inputLabelStyle}>Owner / Agent Mobile *</label>
                <input
                  type="tel"
                  name="ownerMobile"
                  value={formData.ownerMobile}
                  onChange={(e) => setFormData(prev => ({ ...prev, ownerMobile: e.target.value }))}
                  placeholder="e.g., 96541xxxxx"
                  style={{ ...inputStyle }}
                  onFocus={(e) => (e.target.style.borderColor = "#00A79D")}
                  onBlur={(e) => (e.target.style.borderColor = "#E5E7EB")}
                />
                <p style={{ fontSize: 12, color: "#6b7280", marginTop: 6 }}>
                  This number will appear on your listing and will be used for tenant/buyer inquiries.
                </p>
              </div>
              <div style={gridStyle}>
                <div>
